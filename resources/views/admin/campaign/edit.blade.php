...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-300">
    <div class="container mx-auto px-6 py-8">
        <div class="p-6 bg-white rounded-md shadow-md">
            ...
            <form action="{{route('admin.campaign.edit', $campaign->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-6 mt-4">
                    ...
                    <div>
                        ...
                        <select class="w-full border bg-gray-200 focus:bg-white rounded  px-3 py-2 outline-none" name="category_id" >
                            @foreach ($categories as $category)
                                <option class="py-1" value="{{$category->id}}" @if ($campaign->category_id == $category->id)
                                    selected
                        ...
                        @error('category_id')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
                                    <h2 class="text-gray-600 text-sm">{{$message}}</h2>
                                </div>
                            </div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input class="form-input w-full rounded-md bg-gray-200 focus:bg-white p-3" type="number" name="target_donation" value="{{old('target_donation', $campaign->target_donation)}}" placeholder="Target Donasi, Ex: 10000000">
                    ...
                    <div>
                        ...
                        <input type="date" name="max_date" value="{{old('max_date',$campaign->max_date)}}">
                        @error('max_date')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
                                    <p class="text-gray-600 text-sm">{{$message}}</p>
